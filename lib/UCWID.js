...
class UCWID {
    ...
    async ucwid_constructor(clear_cwid,crypto_cwid) {
        ...
        let packet_did = this.cwid_service.cwid_to_did(packet_cwid)
        let u_cwid = n_version + '!' + packet_cwid
        let u_cwid_did = `did:ucwid:${u_cwid}`
        ucwid_packet.did = packet_did
        return [u_cwid,ucwid_packet,u_cwid_did]     
    }
    ...
    async ucwid(data,no_string) {
        ...
        // UCWID formation
        let wrapped_key = await cwraps.key_wrapper(aes_key,this._wrapper_key)  // wrap or derive
        //
        let [u_cwid,ucwid_packet,u_cwid_did] = await this.ucwid_constructor(clear_cwid,crypto_cwid)
        //
        return { "ucwid" : u_cwid, "did" : u_cwid_did, "info" : {
