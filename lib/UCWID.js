...
class UCWID {
    ...
    constructor(conf) {
        ...
        if ( (conf === undefined) || (conf.normalizer === undefined) ) {
            ...
            this.normalizer = (typeof conf.normalizer === 'string') ? require(conf.normalizer) : conf.normalizer
            if ( typeof this.normalizer === "function" ) this.normalizer = conf.normalizer(conf)    
        }
    ...
    async ucwid_constructor(clear_cwid,crypto_cwid,wrapped_key) {
        let ucwid_packet = {
    ...
    async ucwid(data) {
        ...
        let [u_cwid,ucwid_packet] = await this.ucwid_constructor(clear_cwid,crypto_cwid,wrapped_key)
        //
